...
.then(data => {
    ...
    orders.forEach(order=>{
        ...
        order.orderItems.forEach(item => {
            orderItemsHtml += `
                <p><strong>${item.product.description}</strong> - ${item.quantity}pcs @ ksh${item.product.currentprice}</p>
            `;
        });
        ...
        <td>
            <div class="d-flex gap-2">
                <a href="" class="btn btn-light btn-sm update-status" data-id="${order._id}"><iconify-icon icon="solar:eye-broken" class="align-middle fs-18"></iconify-icon></a>
                <a href="" class="btn btn-soft-primary btn-sm update-payment" data-id="${order._id}"><iconify-icon icon="solar:pen-2-broken" class="align-middle fs-18"></iconify-icon></a>
...
document.getElementById("orders-list-body").addEventListener("click", function (event) {
    if (event.target.closest(".update-status")) {
        ...
        updateButton.addEventListener("click", async () => {
            ...
            try {
                const response = await fetch(`/api/f1/orders/${orderId}`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
                        'Authorization': `Bearer ${token}`
...
document.getElementById("orders-list-body").addEventListener("click", function (event) {
    if (event.target.closest(".update-payment")) {
        ...
        updateButton.addEventListener("click", async () => {
            ...
            console.log(selectedStatus)
            try {
                const response = await fetch(`/api/f1/orders/payment/${orderId}`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
                        'Authorization': `Bearer ${token}`
